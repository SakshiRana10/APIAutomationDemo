...
public class ObjectLayerPostAPITeamTest extends MainClassAlaya{
	...
	@Test (dependsOnMethods = {"verifySuccessForPOSTAPIObjectLayer"}, groups = {"test", "dev"}, priority = 0, enabled = true, description = "Verify Success Status for delete team")
	...
	public void verifySuccessForPOSTAPIObjectLayer()
	{
		...
		if(CommonUtilityMethods.getFeatureToggle(team))
		{
        	...
        	ObjectLayerPostAPIPage.verifyHostnameAndSourceDataBaseName(response,team);
        	/*PUT call for object layer on the newly created doc*/
        	ObjectLayerPutAPIPage.verifySuccessPutCallAPIObjectLayer(httpRequest,response,docName,team,"parameterTrue");
	...
	public void verifyAttributesForPOSTAPIObjectLayer()
	{
		...
		if(CommonUtilityMethods.getFeatureToggle(team))
        	ObjectLayerPostAPIPage.verifyattributesforPOSTAPIObjectLayer(httpRequest,team,docName,AttributeNamesForDocStore,AttributeNamesForES);	
		else
		{
		    logger.info("Post call was skipped because system toggle was disabled. Test passed.");
			Assert.assertFalse(CommonUtilityMethods.getFeatureToggle(team),"System toggle is disabled for document Type - "+team);	
	...
	public void verifyResponseForWrongBearerTokenPOSTAPIObjectLayer(){
		RequestSpecification httpRequest= null;
		if(CommonUtilityMethods.getFeatureToggle(team))
        	ObjectLayerPostAPIPage.verifyResponseForInvalidBearerTokenforPOSTAPIObjectLayer(httpRequest,team,docName,payload);
		else
		{
		    logger.info("Post call was skipped because system toggle was disabled. Test passed.");
			Assert.assertFalse(CommonUtilityMethods.getFeatureToggle(team),"System toggle is disabled for document Type - "+team);	
	...
	public void verifyResponseForWrongSessionKeyTypePOSTAPIObjectLayer(){
		RequestSpecification httpRequest= null;
		if(CommonUtilityMethods.getFeatureToggle(team))
        	ObjectLayerPostAPIPage.verifyResponseForInvalidSessionKeyforPOSTAPIObjectLayer(httpRequest,team,docName,payload);
		else
		{
		    logger.info("Post call was skipped because system toggle was disabled. Test passed.");
			Assert.assertFalse(CommonUtilityMethods.getFeatureToggle(team),"System toggle is disabled for document Type - "+team);	
	...
	public void verifyResponseForWrongDocTypePOSTAPIObjectLayer(){
		RequestSpecification httpRequest= null;
		if(CommonUtilityMethods.getFeatureToggle(team))
        	ObjectLayerPostAPIPage.verifyResponseForWrongDocType(httpRequest,team,docName,payload,"POST");
		else
		{
		    logger.info("Post call was skipped because system toggle was disabled. Test passed.");
			Assert.assertFalse(CommonUtilityMethods.getFeatureToggle(team),"System toggle is disabled for document Type - "+team);	
	...
	public void verifyResponseForWrongLocalePOSTAPIObjectLayer(){
		RequestSpecification httpRequest= null;
		if(CommonUtilityMethods.getFeatureToggle(team))
        	ObjectLayerPostAPIPage.verifyResponseForWrongLocale(httpRequest,team,docName,payload,"POST");
		else
		{
		    logger.info("Post call was skipped because system toggle was disabled. Test passed.");
			Assert.assertFalse(CommonUtilityMethods.getFeatureToggle(team),"System toggle is disabled for document Type - "+team);	
	...
	public void verifyAttributesForPutAPIObjectLayer(){
		...
		if(CommonUtilityMethods.getFeatureToggle(team)){
			response=ObjectLayerPage.getCallForObjectyLayer(httpRequest,locale,team,docName,ResponseCodes.SUCCESS).getBody().asString();
			ObjectLayerPutAPIPage.verifySuccessPutCallAPIObjectLayer(httpRequest,response,docName,team,"parameterTrue");
        	ObjectLayerPutAPIPage.verifyattributesforPUTAPIObjectLayer(httpRequest,team,docName,AttributeNamesForDocStore,AttributeNamesForES);	
		}
		else
		{
	...
	public void verifyResponseForWrongDocTypePUTAPIObjectLayer(){
		...
		if(CommonUtilityMethods.getFeatureToggle(team)){
        	...
        	ObjectLayerPostAPIPage.verifyResponseForWrongDocType(httpRequest,team,docName,payloadAfterPut,"PUT");
		}
		else
		{
		    logger.info("Put call was skipped because system toggle was disabled. Test passed.");
	...
	public void verifyResponseForWrongLocalePUTAPIObjectLayer(){
		...
		if(CommonUtilityMethods.getFeatureToggle(team)){
			JSONObject payloadAfterPut = new JSONObject(response);
        	ObjectLayerPostAPIPage.verifyResponseForWrongLocale(httpRequest,team,docName,payloadAfterPut,"PUT");
		}
		else
		{
		    logger.info("Put call was skipped because system toggle was disabled. Test passed.");
	...
	public void verifyResponseForNoParametersPUTAPIObjectLayer(){
		RequestSpecification httpRequest= null;
		if(CommonUtilityMethods.getFeatureToggle(team)){
        	ObjectLayerPutAPIPage.verifySuccessPutCallAPIObjectLayer(httpRequest,response,docName,team,"noParameter");
		}
		else
		{
		    logger.info("Put call was skipped because system toggle was disabled. Test passed.");
